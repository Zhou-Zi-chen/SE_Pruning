...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
        ...
        if format == "default":
            ...
            self.clean_files_all = []
            self.clean_files_all += sorted(glob(join(data_dir, subset, "anechoic", "*.wav")))
            self.clean_files_all += sorted(glob(join(data_dir, subset, "anechoic", "**", "*.wav")))
            self.noisy_files_all = []
            self.noisy_files_all += sorted(glob(join(data_dir, subset, "reverb", "*.wav")))
            self.noisy_files_all += sorted(glob(join(data_dir, subset, "reverb", "**", "*.wav")))
        else:
...
class DatasetSamplerCallback(pl.Callback):
    def __init__(self):
        super().__init__()

    def on_train_epoch_start(self, trainer, pl_module):
        pl_module.data_module.train_set.sample_data_per_epoch()
