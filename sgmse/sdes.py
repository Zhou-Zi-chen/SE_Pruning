...
@SDERegistry.register("sb")
class SBSDE(SDE):
    @staticmethod
    def add_argparse_args(parser):
        parser.add_argument("--N", type=int, default=50, help="The number of timesteps in the SDE discretization. 50 by default")
        parser.add_argument("--noise_schedule", type=str, default="gmax", choices=["gmax", "vp", "ve", "bb"], help="Noise schedule to use. 'gmax' by default.")
        ...
        parser.add_argument("--sigma", type=float, default=1.0, help="Parameter of the diffusion coefficient. 1.0 by default for both BB.")
        parser.add_argument("--eps", type=float, default=1e-8, help="Small constant to avoid numerical instability. 1e-8 by default.")
        parser.add_argument("--sampler_type", type=str, default="ode")
        return parser

    def __init__(self, noise_schedule="gmax", k=2.6, c=0.4, beta_0=0.01, beta_1=20.0, sigma=1.0, N=50, eps=1e-8, sampler_type="ode", **ignored_kwargs):
        """Construct a Schrodinger Bridge with Variance Exploding SDE.
    ...
    def copy(self):
        return SBSDE(noise_schedule=self.noise_schedule, k=self.k, c=self.c, beta_0=self.beta_0, beta_1=self.beta_1, N=self.N)
    ...
    def _sigmas_alphas(self, t):
        if self.noise_schedule == "gmax":
            alpha_t = torch.ones_like(t)
            alpha_T = torch.ones_like(t)
            sigma_t = torch.sqrt(self.beta_0 * t + 0.5 * (self.beta_1 - self.beta_0) * (t ** 2))
            sigma_T = torch.sqrt(torch.tensor(self.beta_0 * self.T + 0.5 * (self.beta_1 - self.beta_0) * (self.T ** 2)))
        elif self.noise_schedule == "vp":
            ...
            sigma_t = torch.sqrt(self.c * (torch.exp(self.beta_0 * t + 0.5 * (self.beta_1 - self.beta_0) * (t ** 2)) - 1))
            sigma_T = torch.sqrt((self.c * (torch.exp(torch.tensor(self.beta_0 * self.T + 0.5 * (self.beta_1 - self.beta_0) * (self.T ** 2))) - 1)))
        elif self.noise_schedule == "ve":
            alpha_t = torch.ones_like(t)
            alpha_T = torch.ones_like(t)
            sigma_t = torch.sqrt((self.c*(self.k**(2*t)-1.0)) / (2*torch.log(torch.tensor(self.k))))
            sigma_T = torch.sqrt((self.c*(self.k**(2*self.T)-1.0)) / (2*torch.log(torch.tensor(self.k))))
        elif self.noise_schedule == "bb":
            alpha_t = torch.ones_like(t)
            alpha_T = torch.ones_like(t)
            sigma_t = torch.sqrt(torch.tensor(1) * t) * self.sigma
...
class BBED(SDE):
    ...
    def __init__(self, T_sampling, k, theta, N=1000, sampler_type='pc', **kwargs):
...
class FM(SDE):
    def add_argparse_args(parser):
        ...
        parser.add_argument("--noise_schedule", type=str, default="ot", choices=["ot", "sb"], help="Noise schedule to use. 'ot' by default.")

    def __init__(self, N=50, sigma_max=1.0, sigma_min=0.01, noise_schedule='ot', **kwargs):
        super().__init__(N)
        self.Tc = 1
        self.sigma_max = sigma_max
        self.sigma_min = sigma_min
    ...
    def _mean(self, x0, y, t):
        time = (t/self.Tc)[:, None, None, None]
        return (1 - time) * y + time * x0

    def _std(self, t):
        if self.noise_schedule == "ot":
            std = t * self.sigma_min + (1 - t) * self.sigma_max
        elif self.noise_schedule == "sb":
            std = torch.sqrt(t * (1 - t)) * self.sigma_max
        return std

    def marginal_prob(self, x0, y, t):
        return self._mean(x0, y, t), self._std(t)

    def vf(self, x, x0, y, t):
        mean, std = self.marginal_prob(x0, y, t)
        if self.noise_schedule == "ot":
            return (self.sigma_min - self.sigma_max) / std[:, None, None, None] * (x - mean) + (x0 - y)
        elif self.noise_schedule == "sb":
            return (1 - 2 * t) / (2 * t * (1 - t)) * (x - mean) + (x0 - y)

    def prior_sampling(self, shape, y):
        if shape != y.shape:
            warnings.warn(f"Target shape {shape} does not match shape of y {y.shape}! Ignoring target shape.")
        std = self._std(torch.zeros((y.shape[0],), device=y.device))
