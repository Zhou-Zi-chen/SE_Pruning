...
def get_sgm_sampler(sde, model, y, **kwargs):
    def ode_sampler():
        with torch.no_grad():
            xt = y
            time_steps = torch.linspace(sde.T, 0, sde.N + 1, device=y.device)
            ...
            for t in time_steps[1:]:
                ...
                # Run DNN
                current_score = model(xt, y, time_prev)
                drift, diffusion = sde.sde(xt, y, time)

                # Update state: weighted sum of previous state, current estimate and prior
                xt = xt + (drift - diffusion ** 2 * current_score) * delta_t
...
def get_sb_sampler(sde, model, y, eps=1e-4, n_steps=50, sampler_type="ode", **kwargs):
    ...
    def sde_sampler():
        ...
        with torch.no_grad():
            ...
            # time_steps = torch.linspace(sde.T, eps, 10 + 1, device=y.device)
            # time_steps = torch.cat([time_steps[0:1], time_steps[-5:]])
...
def get_fm_sampler(sde, model, y, **kwargs):
    # For OT-CFM, this first-order discretization based on Euler's method is
    # completely consistent with the method based on exponential integrators.
    # The proof is given in the supplementary material.
    def ode_sampler():
        with torch.no_grad():
            xt = sde.prior_sampling(y.shape, y)
            time_steps = torch.linspace(1e-4, sde.T, sde.N + 1, device=y.device)
            time_prev = time_steps[0] * torch.ones(xt.shape[0], device=xt.device)
            delta_t = time_steps[1] - time_steps[0]
            ...
            for t in time_steps[1:]:
                ...
                # Update state: weighted sum of previous state, current estimate and prior
                xt = xt + sde.vf(xt, current_estimate, y, time_prev) * delta_t
